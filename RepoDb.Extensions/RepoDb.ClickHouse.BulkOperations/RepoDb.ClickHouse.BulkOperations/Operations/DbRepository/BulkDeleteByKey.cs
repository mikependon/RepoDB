...
{
    /// <summary>
    /// <see cref="DbRepository{TDbConnection}"/> wrapper for the ClickHouse bulk-delete-by-key operation. The
    /// method resolves a connection (reusing the transaction's connection when one is supplied, otherwise
    /// creating one via the repository), delegates to the matching <see cref="ClickHouseConnection"/> extension
    ...
    public static partial class DbRepositoryExtension
    {
        ...
        public static int BulkDeleteByKey<TPrimaryKey>(this DbRepository<ClickHouseConnection> repository,
            string tableName,
        ...
        /// <returns>The number of deleted rows.</returns>
        public static async Task<int> BulkDeleteByKeyAsync<TPrimaryKey>(this DbRepository<ClickHouseConnection> repository,
            string tableName,
            IEnumerable<TPrimaryKey> primaryKeys,
            int? bulkCopyTimeout = null,
            int? batchSize = null,
