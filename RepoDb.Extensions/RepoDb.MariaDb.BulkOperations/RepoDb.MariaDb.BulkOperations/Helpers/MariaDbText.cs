...
{
    /// <summary>
    /// Builds every SQL text used by the MariaDb bulk-operations pseudo-table pipeline. This class was
    /// originally copied from the Oracle provider (PL/SQL anonymous blocks, sequences, <c>MERGE</c>, <c>ROWID</c>)
    /// and has been rewritten here for plain MariaDB, which has none of those:
    /// <list type="bullet">
    /// <item><description>No PL/SQL - "guard block" DDL (<c>CREATE TABLE IF NOT EXISTS</c>, <c>DROP TABLE IF EXISTS</c>)
    /// is native MariaDB syntax, so no wrapper is needed at all.</description></item>
    ...
    /// MariaDB always accepts an explicit value for an <c>AUTO_INCREMENT</c> column, unlike Oracle's
    ...
    /// type in MariaDB (Oracle lets you toggle nullability alone) - <see cref="GetAllowNullForColumnSql"/> looks
    ...
    /// requires <c>AllowUserVariables=True</c> on the connection string - MariaDb defaults this to
    /// <see langword="false"/>.
    /// </summary>
    internal static class MariaDbText
    {
        /// <summary>
        /// Name of the surrogate, always-present ordering column added to every pseudo table (see
        /// <see cref="GetCreatePseudoTableSql"/>). MariaDB has no equivalent to Oracle's implicit <c>ROWID</c>
        ...
        /// re-created (see <see cref="GetCreatePseudoTableSql"/>) before this index is built, and a MariaDB
        ...
        /// they assign it explicitly themselves. <c>MariaDbBulkImportPseudoTableType.Physical</c>
        /// creates an ordinary persistent table; anything else (<c>Memory</c>, or an unresolved <c>Auto</c>)
        /// creates a <c>TEMPORARY</c> table instead - MariaDB's closest equivalent to Oracle's session-private
            ...
            Field qualifierField = null)
        {
            ...
            var temporaryKeyword = pseudoTableType == MariaDbBulkImportPseudoTableType.Physical
                ? string.Empty
                : "TEMPORARY ";
        ...
        /// <summary>
        /// Builds the DDL that drops a pseudo table once a bulk operation is done with it. MariaDB's
        ...
        /// must be made nullable first. Unlike Oracle's <c>ALTER TABLE t MODIFY (col NULL)</c>, MariaDB's
        ...
        /// This replaces Oracle's <c>ALL_TAB_IDENTITY_COLS</c> lookup - MariaDB has no named sequence object to
        /// look up at all, so the two-column, one-row shape the caller expects
        /// (<c>MariaDbExecution.GetIdentitySequenceMetadata</c>) is kept for signature compatibility,
        /// but column 0 now holds the numeric seed (as a string) rather than a sequence name, and column 1 is
        /// always <c>'NO'</c> - MariaDB's <c>AUTO_INCREMENT</c> has no Oracle-style <c>GENERATED ALWAYS</c>
        ...
        /// <c>information_schema.TABLES.AUTO_INCREMENT</c> (the original implementation). MariaDB 8 caches
        ...
        /// way the real <c>AUTO_INCREMENT</c> counter would (harmless: MariaDB never reserves a deleted row's id).
        ...
        /// </summary>
        /// <remarks>
        /// MariaDB's <c>AUTO_INCREMENT</c> has no equivalent to Oracle's per-row <c>SEQUENCE.NEXTVAL</c>, and
        /// relying on <c>LAST_INSERT_ID()</c> plus positional arithmetic after a multi-row
        /// <c>INSERT ... SELECT</c> is not safe under MariaDB 8's default (interleaved) <c>innodb_autoinc_lock_mode</c>,
        ...
        /// fires for these rows at all (MariaDB still auto-advances its internal counter past whatever explicit
        ...
        /// <c>ROWID</c> ordering. <paramref name="isAlwaysGenerated"/> needs no MariaDB equivalent - there is no
        ...
        /// </param>
        /// <param name="isAlwaysGenerated">Unused for MariaDB - kept for signature compatibility with the Oracle-derived caller.</param>
        /// <param name="dbSetting">The current <see cref="IDbSetting"/>.</param>
        ...
        /// </summary>
        /// <remarks>
        /// MariaDB has no <c>MERGE</c> statement, and <c>INSERT ... ON DUPLICATE KEY UPDATE</c> - MariaDB's native
        ...
        /// MariaDb's documented behavior for a batched command.
        /// </remarks>
        ...
        /// (see its remarks for the sequence-seeding caveat) - see the inline steps below. A MariaDB multi-table
        ...
        /// <paramref name="pseudoTableName"/>, via MariaDB's multi-table <c>UPDATE ... INNER JOIN</c> syntax -
        /// the direct MariaDB equivalent of Oracle's <c>MERGE ... WHEN MATCHED THEN UPDATE</c> (there is no
        ...
        /// <paramref name="pseudoTableName"/>, via MariaDB's multi-table <c>DELETE ... INNER JOIN</c> syntax -
        /// much simpler than Oracle's <c>ROWID</c>-based subquery form, since MariaDB supports deleting directly
        /// through a join.
        /// </summary>
