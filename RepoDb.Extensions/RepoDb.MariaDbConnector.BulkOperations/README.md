...
A high-performant extension library of RepoDB that does bulk operations towards a MariaDB database. It stages rows through a pseudo (staging) table and loads them via `RepoDb.Connector.MariaDbConnector`'s `MariaDbBulkCopy`, which itself is built on top of `MySqlConnector`'s native `MySqlBulkCopy` implementation.
