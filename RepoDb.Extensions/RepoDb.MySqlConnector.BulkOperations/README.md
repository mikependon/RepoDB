[![MySqlConnectorBulkBuild](https://img.shields.io/github/actions/workflow/status/mikependon/RepoDB/build-mysqlconnector-bulk.yml?logo=github&label=build%20and%20tests&style=for-the-badge)](https://github.com/mikependon/RepoDB/actions/workflows/build-mysqlconnector-bulk.yml)
[![MySqlConnectorBulkHome](https://img.shields.io/badge/home-github-important?&logo=github&style=for-the-badge)](https://github.com/mikependon/RepoDb)
[![MySqlConnectorBulkVersion](https://img.shields.io/nuget/v/repodb.mysqlconnector.bulkoperations?&logo=nuget&style=for-the-badge)](https://www.nuget.org/packages/RepoDb.MySqlConnector.BulkOperations)

# [RepoDb.MySqlConnector.BulkOperations](https://www.nuget.org/packages/RepoDb.MySqlConnector.BulkOperations)

High-performance bulk operations for RepoDB on MySqlConnector. Row loading goes through ODP.NET's `MySqlConnectorBulkCopy`
...
- [How Rows Are Loaded: MySqlConnectorBulkCopy and the Transaction Boundary](#how-rows-are-loaded-mysqlconnectorbulkcopy-and-the-transaction-boundary)
...
**`mappings`** (`BulkInsert` only) — an explicit list of `MySqlConnectorBulkInsertMapItem` describing which
source properties/columns map to which destination columns, and (optionally) which
`MySqlConnector.ManagedDataAccess.Client.MySqlConnectorDbType` to bind each one as. When omitted, the matching
properties/columns from the target table are used automatically, honoring each property's
`[MySqlConnectorDbType]`/`[MySqlConnectorDbTypeEx]` attribute exactly like the rest of this MySqlConnector provider. The explicit
`MySqlConnectorDbType` override only takes effect when `identityBehavior: ReturnIdentity` forces the array-bind
path (see [How Rows Are Loaded](#how-rows-are-loaded-mysqlconnectorbulkcopy-and-the-transaction-boundary)) -
`MySqlConnectorBulkCopy` has no equivalent per-column type override and instead infers the wire type from each
...
- `ReturnIdentity` — the database-generated (or matched) identity value is read back and written onto
  each entity/row. On `BulkInsert`, requesting this switches the row-load itself from `MySqlConnectorBulkCopy`
  back to array binding with `RETURNING ... INTO`, since `MySqlConnectorBulkCopy` has no way to report back
  generated values - see [How Rows Are Loaded](#how-rows-are-loaded-mysqlconnectorbulkcopy-and-the-transaction-boundary).

**`pseudoTableType`** (`BulkMerge`, `BulkUpdate`, `BulkDelete` only) — an `MySqlConnectorBulkImportPseudoTableType`
...
Unlike the PostgreSQL bulk package, there is no `BulkImportMergeCommandType` (MySqlConnector has exactly one
native upsert construct, `MERGE INTO`).

## How Rows Are Loaded: MySqlConnectorBulkCopy and the Transaction Boundary

Every other provider's bulk insert loads rows into a staging table first. MySqlConnector's `BulkInsert` skips
...
- **`MySqlConnectorBulkCopy`** *(the default, used whenever identities aren't being returned)* - ODP.NET's genuine
...
- **Array binding with `RETURNING ... INTO`** *(only when `identityBehavior: ReturnIdentity` is requested
  on `BulkInsert`)* - `MySqlConnectorBulkCopy` has no mechanism to report back generated or matched values, so
...
there is exactly one "write rows into an MySqlConnector table" code path in this package, and every bulk operation
uses it. Staging loads never request `ReturnIdentity` (identity correlation for `BulkMerge` is handled
separately - see [BulkMerge](#bulkmerge)), so they always go through `MySqlConnectorBulkCopy`.

**The transaction boundary this creates.** Per MySqlConnector's own ODP.NET documentation, *"all bulk copy
operations are agnostic of any local or distributed transaction created by the application"* -
`MySqlConnectorBulkCopy` has no constructor or property that accepts an `MySqlConnectorTransaction`, and rows it writes
...
- The row-load step itself (the `MySqlConnectorBulkCopy` call) does **not** - if your transaction is later rolled
...
set-based `MERGE INTO` / `DELETE ... WHERE EXISTS` statement against it. MySqlConnector's `CREATE TABLE` and
...
- **`Physical`** — `CREATE TABLE ... AS SELECT ...`, an ordinary heap table. It carries **no per-session
  ...
  your MySqlConnector environment attaches to GTTs. `Memory` and `Physical` staging tables for the same real
...
pass.** `MySqlConnectorBulkCopy.WriteToServer` (see [How Rows Are Loaded](#how-rows-are-loaded-mysqlconnectorbulkcopy-and-the-transaction-boundary))
always performs a direct-path load internally, and MySqlConnector's direct-path engine cannot write into a Global
Temporary Table at all - this fails live with `ORA-39826: Direct path load of view or synonym (...) could
not be resolved`, MySqlConnector's generic error for a direct-path destination it can't support. Since the staging
table is always loaded via `MySqlConnectorBulkCopy`, a GTT-backed staging table can never actually receive data as
...
`MySqlConnectorBulkCopy`). This means the concurrency caveat above for `Physical` currently applies unconditionally,
...
```csharp
using (var connection = new MySqlConnectorConnection(ConnectionString))
{
...
using (var connection = new MySqlConnectorConnection(ConnectionString))
{
    ...
    connection.BulkInsert<Customer>(customers, identityBehavior: MySqlConnectorBulkImportIdentityBehavior.ReturnIdentity);
...
`BulkMerge` never uses MySqlConnector's `RETURNING` clause on the `MERGE` statement itself (that's only supported
starting with MySqlConnector Database 23ai). When `identityBehavior: MySqlConnectorBulkImportIdentityBehavior.ReturnIdentity` is
...
using (var connection = new MySqlConnectorConnection(ConnectionString))
{
    ...
    var mergedRows = connection.BulkMerge<Customer>(customers, pseudoTableType: MySqlConnectorBulkImportPseudoTableType.Physical);
}
```
