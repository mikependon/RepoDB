...
namespace RepoDb.MySqlConnector.BulkOperations
{
    ...
    /// <remarks>
    /// <para>
    /// Needed because <see cref="MySqlConnector.MySqlBulkCopy"/> does not skip reading a source column just
    /// because it has no entry in <see cref="MySqlConnector.MySqlBulkCopy.ColumnMappings"/>. Internally, it
    ...
    /// matching destination column is therefore still handed to MySqlConnector's value serializer even though
    /// its destination mapping was deliberately left out - and if that value's CLR type isn't one of the
    /// handful MySqlConnector's serializer recognizes (a plain scalar, string, <see cref="Guid"/>, etc.), it
    ...
    /// </para>
    /// <para>
    /// The fix is to never let <see cref="MySqlConnector.MySqlBulkCopy"/> see those extra columns at all: this
    ...
    /// "extra" ordinal left for MySqlConnector to fall back to ignoring (and serializing anyway).
    /// </para>
    /// </remarks>
    internal sealed class ColumnFilteredDataReader : IDataReader
    {
        ...
        /// out-of-bounds read throws mid-<c>LOAD DATA LOCAL INFILE</c> transfer, which leaves the MySqlConnector
