...
{
    /// <summary>
    /// <see cref="DbRepository{TDbConnection}"/> wrapper for the Oracle bulk-delete-by-key operation. The
    /// method resolves a connection (reusing the transaction's connection when one is supplied, otherwise
    /// creating one via the repository), delegates to the matching <see cref="OracleConnection"/> extension
