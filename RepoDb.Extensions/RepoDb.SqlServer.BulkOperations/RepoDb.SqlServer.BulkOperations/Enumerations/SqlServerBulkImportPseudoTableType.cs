...
namespace RepoDb.Enumerations.SqlServer
{
    ...
    public enum SqlServerBulkImportPseudoTableType : short
    {
        ...
        /// than or equal to <see cref="SqlServerConstants.RowCountThresholdForPhysicalTable"/> (<c>5,000</c>),
        ...
        /// operation writes staged rows via <see cref="SqlServer.ManagedDataAccess.Client.SqlServerBulkCopy"/>,
        /// which always performs a direct-path load internally (ODP.NET has no conventional-path
        /// alternative) - and SqlServer's direct-path engine cannot write into a Global Temporary Table at
        ...
        /// writing to the GTT via array-bound <c>INSERT</c>s instead of <c>SqlServerBulkCopy</c>) or is fully
        ...
        /// avoiding whatever session-temporary-object overhead your SqlServer environment attaches to GTTs.
