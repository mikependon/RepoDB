...
namespace RepoDb.PostgreSql.IntegrationTests
{
    public static class Helper
    {
        ...
        public static List<CompleteTable> CreateCompleteTables(int count)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                tables.Add(new CompleteTable
                {
                    ...
                    ColumnDate = DateOnly.FromDateTime(now),
                    ColumnDateAsArray = new[] { DateOnly.FromDateTime(now), DateOnly.FromDateTime(now), DateOnly.FromDateTime(now) },
        ...
        public static void UpdateCompleteTableProperties(CompleteTable table)
        {
            ...
            table.ColumnDate = DateOnly.FromDateTime(now);
            table.ColumnDateAsArray = new[] { DateOnly.FromDateTime(now), DateOnly.FromDateTime(now), DateOnly.FromDateTime(now) };
        ...
        public static List<ExpandoObject> CreateCompleteTablesAsExpandoObjects(int count)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                item["ColumnDate"] = DateOnly.FromDateTime(now);
                item["ColumnDateAsArray"] = new[] { DateOnly.FromDateTime(now), DateOnly.FromDateTime(now), DateOnly.FromDateTime(now) };
