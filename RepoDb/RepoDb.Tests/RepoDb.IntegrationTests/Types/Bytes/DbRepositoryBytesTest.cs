    ...
    public class DbRepositoryBytesTest
    {
        ...
        public void TestDbRepositoryBytesCrudViaTableName()
        {
            ...
            using (var repository = new DbRepository<SqlConnection>(Database.ConnectionStringForRepoDb))
            {
                ...
                // Act Query
                var data = repository.Query(ClassMappedNameCache.Get<BytesClass>(DbSettingMapper.Get(typeof(SqlConnection))), new { SessionId = (Guid)id }).FirstOrDefault();
        ...
        public void TestDbRepositoryBytesNullCrudViaTableName()
        {
            ...
            {
                // Act Insert
                var id = repository.Insert(ClassMappedNameCache.Get<BytesClass>(DbSettingMapper.Get(typeof(SqlConnection))), entity);
        ...
        public void TestDbRepositoryBytesCrudViaTableNameAsync()
        {
            ...
            {
                // Act Insert
                var insertResult = repository.InsertAsync(ClassMappedNameCache.Get<BytesClass>(DbSettingMapper.Get(typeof(SqlConnection))), entity);
                var id = insertResult.Result;

                // Act Query
                var queryResult = repository.QueryAsync(ClassMappedNameCache.Get<BytesClass>(DbSettingMapper.Get(typeof(SqlConnection))), new { SessionId = (Guid)id });
                var data = queryResult.Result.FirstOrDefault();
