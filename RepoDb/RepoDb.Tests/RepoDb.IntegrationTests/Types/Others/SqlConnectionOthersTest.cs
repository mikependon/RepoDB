...
namespace RepoDb.IntegrationTests.Types.Others
{
    ...
    public class SqlConnectionOthersTest
    {
        ...
        public void TestSqlConnectionOthersCrudViaTableName()
        {
            ...
            {
                // Act Insert
                var id = connection.Insert(ClassMappedNameCache.Get<OthersClass>(DbSettingMapper.Get(typeof(SqlConnection))), entity);

                // Act Query
                var data = connection.Query(ClassMappedNameCache.Get<OthersClass>(DbSettingMapper.Get(typeof(SqlConnection))), new { SessionId = (Guid)id }).FirstOrDefault();
        ...
        public void TestSqlConnectionOthersCrudViaTableNameAsync()
        {
            ...
            {
                // Act Insert
                var insertResult = connection.InsertAsync(ClassMappedNameCache.Get<OthersClass>(DbSettingMapper.Get(typeof(SqlConnection))), entity);
                var id = insertResult.Result;

                // Act Query
                var queryResult = connection.QueryAsync(ClassMappedNameCache.Get<OthersClass>(DbSettingMapper.Get(typeof(SqlConnection))), new { SessionId = (Guid)id });
                var data = queryResult.Result.FirstOrDefault();
