...
RepoDB takes security seriously, but is honest about what that means in practice: RepoDB is primarily maintained by a single individual, not a security team or a vendor with an SLA (see our [Support Policy](SUPPORT_POLICY.md) and the [Governance](SUPPORT_POLICY.md#governance) section). This document explains what is and isn't covered, how to report a vulnerability, and how we mitigate the security risks specific to a data-access library.
...
3. ⏱️ You'll get an acknowledgment during the hours defined in the [Support Policy](SUPPORT_POLICY.md) (CET timezone; single maintainer — there is no 24/7 security desk).
