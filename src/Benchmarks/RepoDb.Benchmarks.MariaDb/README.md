# 🦭 RepoDb.Benchmarks.MariaDb

Benchmarks comparing RepoDB against Dapper, Entity Framework Core, and Linq2Db on MariaDB. See the [main Benchmarks README](../README.md) for the full methodology, ORM list, and Enterprise Notice.

## ❓ Why this Benchmark?

This benchmark exists to give **visibility** into how RepoDB performs against other widely used .NET ORMs on MariaDB, and to do so in a way that avoids bias — the same schema, the same dataset, and the same operations are run against every ORM in a single pass, and all of the benchmarking code is open for anyone to read or challenge.
...
2. Run the benchmark project in `Release` configuration:
   ...
   ```bash
   cd src/Shared/RepoDb.Benchmarks/RepoDb.Benchmarks.MariaDb
...
RepoDB, Dapper, and the schema/seed logic in `DatabaseHelper` all go through [RepoDb.MariaDb](../../../Providers/RepoDb.MariaDb) and [RepoDb.MariaDb.BulkOperations](../../../Providers/RepoDb.MariaDb.BulkOperations), built on top of [RepoDb.Connector.MariaDb](https://www.nuget.org/packages/RepoDb.Connector.MariaDb) (which itself wraps `MySql.Data`), exposing `MariaDbConnection`.

Linq2Db targets MariaDB explicitly via `MySqlVersion.MariaDB10` with `MySqlProvider.MySqlData`.

Entity Framework Core uses [Pomelo.EntityFrameworkCore.MySql](https://www.nuget.org/packages/Pomelo.EntityFrameworkCore.MySql) — the only EF Core provider that officially supports MariaDB (Oracle's `MySql.EntityFrameworkCore` does not). Pomelo is built on [MySqlConnector](https://mysqlconnector.net/) rather than `MySql.Data`, so this is the one ORM in this project that talks to the server through a different client library than the rest. Pomelo also has not yet released EF Core 10 support, so this project's `Microsoft.EntityFrameworkCore`/EF Core packages are pinned to **9.x**, one major version behind the 10.0.11 used by the other benchmark projects. Once Pomelo ships EF Core 10 support, this pin should be lifted to stay in line with the rest of the suite.
