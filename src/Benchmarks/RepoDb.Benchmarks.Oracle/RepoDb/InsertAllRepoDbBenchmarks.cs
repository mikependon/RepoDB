...
namespace RepoDb.Benchmarks.Oracle.RepoDb
{
    public class InsertAllRepoDbBenchmarks : RepoDbBaseBenchmarks
    {
        ...
        {
            using var connection = GetConnection().EnsureOpen() as OracleConnection;
        ...
        {
            await using var connection = await GetConnection().EnsureOpenAsync().ConfigureAwait(false) as OracleConnection;
