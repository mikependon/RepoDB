...
namespace RepoDb.Benchmarks.PostgreSql.RepoDb
{
    public class InsertAllRepoDbBenchmarks : RepoDbBaseBenchmarks
    {
        ...
        {
            using var connection = GetConnection().EnsureOpen() as NpgsqlConnection;
        ...
        {
            await using var connection = await GetConnection().EnsureOpenAsync().ConfigureAwait(false) as NpgsqlConnection;
