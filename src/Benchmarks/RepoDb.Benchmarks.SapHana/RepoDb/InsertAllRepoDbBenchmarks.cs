...
namespace RepoDb.Benchmarks.SapHana.RepoDb
{
    public class InsertAllRepoDbBenchmarks : RepoDbBaseBenchmarks
    {
        ...
        {
            using var connection = GetConnection().EnsureOpen() as HanaConnection;
        ...
        {
            await using var connection = await GetConnection().EnsureOpenAsync().ConfigureAwait(false) as HanaConnection;
