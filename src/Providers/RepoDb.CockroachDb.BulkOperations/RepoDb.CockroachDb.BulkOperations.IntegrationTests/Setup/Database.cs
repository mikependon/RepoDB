...
namespace RepoDb.IntegrationTests.Setup
{
    ...
    public static class Database
    {
        ...
        /// <summary>
        /// Gets the connection string used to reach the CockroachDB cluster itself (the <c>defaultdb</c>
        /// database - CockroachDB's default database, unlike plain PostgreSQL's <c>postgres</c>) - used
        /// only to create the target database if it does not already exist.
        /// </summary>
        public static string ConnectionStringForSystem { get; private set; }

        /// <summary>
        /// Gets the connection string to be used for the CockroachDB test database.
        /// </summary>
        ...
        /// <c>CREATE DATABASE IF NOT EXISTS</c>, Postgres/CockroachDB has no such guarded form - creating a
        /// database is done against the <c>defaultdb</c> database, and a duplicate name is a
        ...
        /// CockroachDB's native identity column, the direct equivalent of MariaDB's
        /// <c>AUTO_INCREMENT</c> and reported back by <c>RETURNING</c> (see
        /// <c>CockroachDbText.GetInsertFromPseudoTableForReturnIdentitySql</c>) rather than a pre-assigned
