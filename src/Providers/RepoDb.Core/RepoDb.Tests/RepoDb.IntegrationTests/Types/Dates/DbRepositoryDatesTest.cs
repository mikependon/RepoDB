    ...
    public class DbRepositoryDatesTest
    {
        ...
        public async Task TestDbRepositoryDatesCrudAsync2()
        {
            ...
            using (var repository = new DbRepository<SqlConnection>(Database.ConnectionString))
            {
                ...
                // Act Query
                var data = (await repository.QueryAsync<DatesClass>(e => e.SessionId == (Guid)id).ConfigureAwait(false)).FirstOrDefault();
        ...
        public async Task TestDbRepositoryDatesMappedCrudAsync2()
        {
            ...
            using (var repository = new DbRepository<SqlConnection>(Database.ConnectionString))
            {
                ...
                // Act Query
                var data = (await repository.QueryAsync<DatesMapClass>(e => e.SessionId == (Guid)id).ConfigureAwait(false)).FirstOrDefault();
        ...
        public async Task TestDbRepositoryDatesCrudViaTableNameAsync2()
        {
            ...
            {
                // Act Insert
                var id = await repository.InsertAsync(ClassMappedNameCache.Get<DatesClass>(), entity).ConfigureAwait(false);

                // Act Query
                var data = (await repository.QueryAsync(ClassMappedNameCache.Get<DatesClass>(), new { SessionId = (Guid)id }).ConfigureAwait(false)).FirstOrDefault();
