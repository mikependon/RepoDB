...
{
    /// <summary>
    /// <see cref="DbRepository{TDbConnection}"/> wrappers for the Db2 bulk-merge operation. Each method
    ...
    public static partial class DbRepositoryExtension
    {
        ...
        public static int BulkMerge<TEntity>(this DbRepository<DB2Connection> repository,
            ...
            where TEntity : class =>
            repository.BulkMerge(ClassMappedNameCache.Get<TEntity>(), entities, qualifiers, mappings, bulkCopyOptions, bulkCopyTimeout, batchSize, identityBehavior, pseudoTableType, trace, traceKey, transaction);
            ...
            where TEntity : class
        {
            ...
            try
            {
                return connection.BulkMerge(tableName ?? ClassMappedNameCache.Get<TEntity>(), entities, qualifiers != null ? Field.Parse(qualifiers) : null, mappings, bulkCopyOptions, bulkCopyTimeout, batchSize, identityBehavior, pseudoTableType, trace, traceKey, transaction);
            }
            finally
            {
        ...
        /// <returns>The number of affected rows.</returns>
        public static async Task<int> BulkMergeAsync<TEntity>(this DbRepository<DB2Connection> repository,
            IEnumerable<TEntity> entities,
            Expression<Func<TEntity, object>> qualifiers = null,
            IEnumerable<Db2BulkInsertMapItem> mappings = null,
            DB2BulkCopyOptions bulkCopyOptions = default,
            ...
            where TEntity : class =>
            await repository.BulkMergeAsync(ClassMappedNameCache.Get<TEntity>(), entities, qualifiers, mappings, bulkCopyOptions, bulkCopyTimeout, batchSize, identityBehavior, pseudoTableType, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
            ...
            where TEntity : class
        {
            ...
            try
            {
                return await connection.BulkMergeAsync(tableName ?? ClassMappedNameCache.Get<TEntity>(), entities, qualifiers != null ? Field.Parse(qualifiers) : null, mappings, bulkCopyOptions, bulkCopyTimeout, batchSize, identityBehavior, pseudoTableType, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
            }
            finally
            {
                DisposeIfOwned(repository, transaction, connection);
