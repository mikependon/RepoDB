...
{
    /// <summary>
    /// A property handler that maps the Db2 <c>XML</c> type into an <see cref="XElement"/> property.
    /// </summary>
    public class Db2XmlToXElementPropertyHandler : IPropertyHandler<object, XElement>
    {
        /// <summary>
        /// Converts the Db2 <c>XML</c> value, as returned by the driver, into an <see cref="XElement"/>.
        /// </summary>
        ...
        /// <summary>
        /// Converts an <see cref="XElement"/> into its unformatted XML text, to be written into a Db2 <c>XML</c> column. The text is bound as a string, so the parameter has to be accepted by Db2 for the <c>XML</c> column (for example by typing it as <c>DB2Type.Xml</c>).
        /// </summary>
