...
{
    /// <summary>
    /// <see cref="DbRepository{TDbConnection}"/> wrappers for the DuckDB bulk-merge operation. Each method
    /// resolves a connection (reusing the transaction's connection when one is supplied, otherwise creating
    /// one via the repository), delegates to the matching <see cref="DuckDBConnection"/> extension method,
    ...
    public static partial class DbRepositoryExtension
    {
        ...
        public static int BulkMerge<TEntity>(this DbRepository<DuckDBConnection> repository,
        ...
        public static int BulkMerge<TEntity>(this DbRepository<DuckDBConnection> repository,
            ...
            where TEntity : class
        {
            var connection = (DuckDBConnection)transaction?.Connection ?? repository.CreateConnection();
        ...
        /// <returns>The number of affected rows.</returns>
        public static async Task<int> BulkMergeAsync<TEntity>(this DbRepository<DuckDBConnection> repository,
            IEnumerable<TEntity> entities,
            Expression<Func<TEntity, object>> qualifiers = null,
            IEnumerable<DuckDbBulkInsertMapItem> mappings = null,
            int? bulkCopyTimeout = null,
            ...
            where TEntity : class =>
            await repository.BulkMergeAsync(ClassMappedNameCache.Get<TEntity>(), entities, qualifiers, mappings, bulkCopyTimeout, batchSize, identityBehavior, pseudoTableType, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
            ...
            where TEntity : class
        {
            ...
            try
            {
                return await connection.BulkMergeAsync(tableName ?? ClassMappedNameCache.Get<TEntity>(), entities, qualifiers != null ? Field.Parse(qualifiers) : null, mappings, bulkCopyTimeout, batchSize, identityBehavior, pseudoTableType, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
            }
            finally
            {
                DisposeIfOwned(repository, transaction, connection);
