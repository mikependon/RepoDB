...
{
    /// <summary>
    /// <see cref="DbRepository{TDbConnection}"/> wrappers for the DuckDB bulk-update operation. Each method
    ...
    public static partial class DbRepositoryExtension
    {
        ...
        public static int BulkUpdate<TEntity>(this DbRepository<DuckDBConnection> repository,
        ...
        /// <returns>The number of updated rows.</returns>
        public static async Task<int> BulkUpdateAsync<TEntity>(this DbRepository<DuckDBConnection> repository,
            IEnumerable<TEntity> entities,
            Expression<Func<TEntity, object>> qualifiers = null,
            IEnumerable<DuckDbBulkInsertMapItem> mappings = null,
            int? bulkCopyTimeout = null,
            ...
            where TEntity : class =>
            await repository.BulkUpdateAsync(ClassMappedNameCache.Get<TEntity>(), entities, qualifiers, mappings, bulkCopyTimeout, batchSize, pseudoTableType, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
            ...
            where TEntity : class
        {
            ...
            try
            {
                return await connection.BulkUpdateAsync(tableName ?? ClassMappedNameCache.Get<TEntity>(), entities, qualifiers != null ? Field.Parse(qualifiers) : null, mappings, bulkCopyTimeout, batchSize, pseudoTableType, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
            }
            finally
            {
                DisposeIfOwned(repository, transaction, connection);
