...
namespace RepoDb.DbSettings
{
    ...
    public sealed class DuckDbDbSetting : BaseDbSetting
    {
        ...
        public DuckDbDbSetting()
        {
            ...
            OpeningQuote = "\"";
            // DuckDB's native placeholder syntax is '$name', not '@name' (confirmed empirically: '@name'
            // either fails to bind or silently matches every row depending on context, since DuckDB's own
            // parser does not recognize '@' as a parameter sigil at all).
            ParameterPrefix = "$";
