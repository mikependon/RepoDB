...
{
    /// <summary>
    /// A property handler that maps the Firebird <c>DECFLOAT(16)</c> and <c>DECFLOAT(34)</c> types into a <see cref="Nullable{T}"/> of <see cref="decimal"/> property. A <see cref="decimal"/> cannot hold every <c>DECFLOAT</c> value (34 digits, wide exponent, <c>NaN</c> and infinity), so such values are rejected instead of being silently altered.
    /// </summary>
