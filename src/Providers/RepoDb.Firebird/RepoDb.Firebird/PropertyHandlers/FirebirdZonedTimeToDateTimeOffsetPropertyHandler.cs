...
{
    /// <summary>
    /// A property handler that maps the Firebird <c>TIME WITH TIME ZONE</c> type into a <see cref="DateTimeOffset"/> property, as no single BCL type represents a time of day with an offset. Only the time of day and the UTC offset are meaningful: the date part is always 1970-01-01, and the name of the time zone is not kept.
    /// </summary>
    /// <remarks>
    /// Use <see cref="FirebirdZonedTimeToNullableDateTimeOffsetPropertyHandler"/> for a <see cref="Nullable{T}"/> of <see cref="DateTimeOffset"/> property.
    /// </remarks>
    public class FirebirdZonedTimeToDateTimeOffsetPropertyHandler : IPropertyHandler<object, DateTimeOffset>
    {
        /// <summary>
        /// Converts the <c>TIME WITH TIME ZONE</c> value, as returned by the driver, into a <see cref="DateTimeOffset"/> on 1970-01-01.
        /// </summary>
        ...
        public DateTimeOffset Get(object input,
            ...
            FirebirdZonedValueConverter.ToDateTimeOffset(input) ?? default;
        ...
        /// <returns>The <c>FbZonedTime</c> (boxed).</returns>
        public object Set(DateTimeOffset input,
            PropertyHandlerSetOptions options) =>
            FirebirdZonedValueConverter.ToZonedTime(input);
    }
}
