...
{
    /// <summary>
    /// A property handler that maps the MariaDB spatial types (<c>GEOMETRY</c>, <c>POINT</c>, <c>LINESTRING</c>, <c>POLYGON</c>, <c>MULTIPOINT</c>, <c>MULTILINESTRING</c>, <c>MULTIPOLYGON</c> and <c>GEOMETRYCOLLECTION</c>) into a <see cref="MySqlGeometry"/> property.
    /// </summary>
    public class MariaDbGeometryToMySqlGeometryPropertyHandler : IPropertyHandler<object, MySqlGeometry>
    {
        /// <summary>
        /// Converts the spatial value, as returned by the driver, into a <see cref="MySqlGeometry"/>.
        /// </summary>
        /// <param name="input">The spatial value of the column, as returned by the driver (a <see cref="MySqlGeometry"/>, or a <see cref="byte"/> array in the MySQL/MariaDB internal format, which is a 4-byte SRID followed by the WKB).</param>
        ...
        /// <returns>The <see cref="byte"/> array of the <see cref="MySqlGeometry.Value"/> (the MySQL/MariaDB internal format, which is a 4-byte SRID followed by the WKB), or <c>null</c> when its <see cref="MySqlGeometry.IsNull"/> is <c>true</c>.
