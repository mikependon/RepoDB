...
namespace RepoDb
{
    public static partial class MySqlConnectorConnectionExtension
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                // Bulk and post process - the pseudo table only ever needs the one qualifier column
                await MySqlConnectorExecution.CreatePseudoTableAsync(connection, tableName, pseudoTableName, pseudoTableType, qualifierField, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
                await MySqlConnectorExecution.CreatePseudoTableIndexAsync(connection, pseudoTableName, new[] { qualifierField }, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
                ...
                // Execute and return
                result = await MySqlConnectorExecution.DeleteFromPseudoTableAsync(connection, tableName, pseudoTableName, new[] { qualifierField }, trace, traceKey, transaction, cancellationToken).ConfigureAwait(false);
            }
            finally
            {
                // Drop the pseudo table
