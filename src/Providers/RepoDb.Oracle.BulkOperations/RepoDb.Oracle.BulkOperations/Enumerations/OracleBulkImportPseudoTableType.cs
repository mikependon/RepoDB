...
namespace RepoDb.Enumerations.Oracle
{
    ...
    public enum OracleBulkImportPseudoTableType : short
    {
        ...
        /// operation writes staged rows via <see cref="Oracle.ManagedDataAccess.Client.OracleBulkCopy"/>,
        /// which always performs a direct-path load internally (ODP.NET has no conventional-path
        /// alternative) - and Oracle's direct-path engine cannot write into a Global Temporary Table at
        ...
        /// writing to the GTT via array-bound <c>INSERT</c>s instead of <c>OracleBulkCopy</c>) or is fully
