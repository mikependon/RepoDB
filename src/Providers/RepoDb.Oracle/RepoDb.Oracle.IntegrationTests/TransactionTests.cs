...
namespace RepoDb.Oracle.IntegrationTests
{
    ...
    public class TransactionTests
    {
         ...
         *
         * NOTE ON InsertAll/MergeAll/UpdateAll: OracleDbSetting.IsMultiStatementExecutable is false, so
         ...
         * same OracleConnection/transaction pair - so commit/rollback semantics are unaffected.
         */
        ...
        public void TestDbTransactionForUpdateAsRolledBack()
        {
            ...
            using (var connection = new OracleConnection(Database.ConnectionString))
            {
                // Act
                connection.Insert<CompleteTable>(entity);
                var originalColumnVarchar = entity.ColumnVarchar;
        ...
        public void TestDbTransactionForUpdateAllAsRolledBack()
        {
            ...
            using (var connection = new OracleConnection(Database.ConnectionString))
            {
                // Act
                connection.InsertAll<CompleteTable>(entities);
                var originalValues = entities.ToDictionary(e => e.Id, e => e.ColumnVarchar);
        ...
        // RISK: unlike the DbTransaction-based tests above (which use OracleConnection.BeginTransaction()
        // directly and are known-good patterns for ODP.NET), these rely on System.Transactions ambient
        // transaction enlistment. Oracle.ManagedDataAccess.Core supports enlisting in a TransactionScope
        // for a *single* connection/resource (no MSDTC / distributed coordinator needed in that case),
        // but this has not been verified against the actual Docker Oracle instance used by this suite.
        // If these fail while the DbTransaction region above passes, suspect TransactionScope enlistment
        // support in the installed Oracle.ManagedDataAccess.Core version specifically, not a RepoDb bug.
