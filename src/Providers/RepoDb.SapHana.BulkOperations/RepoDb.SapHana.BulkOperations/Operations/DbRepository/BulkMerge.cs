...
{
    /// <summary>
    /// <see cref="DbRepository{TDbConnection}"/> wrappers for the SapHana bulk-merge operation. Each method
    /// resolves a connection (reusing the transaction's connection when one is supplied, otherwise creating
    /// one via the repository), delegates to the matching <see cref="HanaConnection"/> extension method,
    ...
    public static partial class DbRepositoryExtension
    {
        ...
        public static int BulkMerge<TEntity>(this DbRepository<HanaConnection> repository,
        ...
        /// <returns>The number of affected rows.</returns>
        public static async Task<int> BulkMergeAsync<TEntity>(this DbRepository<HanaConnection> repository,
            IEnumerable<TEntity> entities,
            Expression<Func<TEntity, object>> qualifiers = null,
            IEnumerable<SapHanaBulkInsertMapItem> mappings = null,
            int? bulkCopyTimeout = null,
