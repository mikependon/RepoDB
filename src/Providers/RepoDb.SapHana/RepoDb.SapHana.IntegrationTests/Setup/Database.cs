...
namespace RepoDb.SapHana.IntegrationTests.Setup
{
    public static class Database
    {
        ...
        /// <summary>
        /// Gets a value indicating whether the target SAP HANA instance supports the <c>REAL_VECTOR</c> type.
        /// </summary>
        public static bool IsRealVectorSupported { get; private set; }
        ...
        /// <summary>
        /// Creates the "PropertyHandler" table used to test the property handlers. The <c>REAL_VECTOR</c>
        /// column is added separately as not all SAP HANA versions support it.
        /// </summary>
        private static void CreatePropertyHandlerTable()
        {
            using (var connection = new HanaConnection(ConnectionString))
            {
                if (!TableExists(connection, "PropertyHandler"))
                {
                    connection.ExecuteNonQuery(@"CREATE TABLE ""PropertyHandler""
                        (
                            ""Id"" BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
                            ""ColumnGuid"" NVARCHAR(36) DEFAULT NULL,
                            PRIMARY KEY(""Id"")
                        );");
                }

                var hasVectorColumn = connection.ExecuteScalar<int>(
                    "SELECT COUNT(*) FROM SYS.TABLE_COLUMNS WHERE SCHEMA_NAME = CURRENT_SCHEMA AND TABLE_NAME = 'PropertyHandler' AND COLUMN_NAME = 'ColumnRealVector'") > 0;
                if (!hasVectorColumn)
                {
                    try
                    {
                        connection.ExecuteNonQuery(@"ALTER TABLE ""PropertyHandler"" ADD (""ColumnRealVector"" REAL_VECTOR(3) DEFAULT NULL);");
                        hasVectorColumn = true;
                    }
                    catch (HanaException)
                    {
                        // The REAL_VECTOR type is not supported by this SAP HANA instance.
                    }
                }
                IsRealVectorSupported = hasVectorColumn;
            }
        }

        /// <summary>
        /// Marks the running test as inconclusive when the target SAP HANA instance does not support the <c>REAL_VECTOR</c> type.
        /// </summary>
        public static void EnsureRealVectorSupported()
        {
            if (!IsRealVectorSupported)
            {
                Microsoft.VisualStudio.TestTools.UnitTesting.Assert.Inconclusive("The REAL_VECTOR type is not supported by the target SAP HANA instance.");
            }
        }
